...
static int lua_waitVblankStart(lua_State *L)
{
	...
	if (argc == 0 || t != LUA_TNUMBER) {
		...
		int i;
		for (i = 0; i < count; i++) graphicWaitVblankStart();
	}
	return 0;
}
...
static int Image_save (lua_State *L) {
	...
	{       
		saveImage(filename, getVramDrawBuffer(), SCREEN_WIDTH, SCREEN_HEIGHT, PS2_LINE_SIZE, 0);
	}
	return 0;
}
...
void luaGraphics_init(lua_State *L) {
	...
	theScreenImage.textureWidth  = 480;
	theScreenImage.textureHeight = 272;
